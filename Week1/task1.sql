...
JOIN Person.BusinessEntityAddress bea ON c.CustomerID =
...
JOIN Person.BusinessEntityAddress bea ON soh.ShipToAddressID =
bea.BusinessEntityID
JOIN Person.Address a ON bea.AddressID = a.AddressID
...
-- 16. Get the orderID and total quantity for each order that has a total quantity
of greater than 300:
SELECT
 SalesOrderID,
...
LEFT JOIN
 HumanResources.Employee AS Emp2 ON
Emp1.OrganizationNode.GetAncestor(1) = Emp2.OrganizationNode
LEFT JOIN
...
-- 29. List of orders placed by best customer:  --identify the best customer.
USE AdventureWork;
...
-- 30. List of orders placed by customer who do not have a fax number: USE AdventureWork;
SELECT
 soh.SalesOrderID,
 soh.OrderDate,
 soh.TotalDue,
...
-- 31. List of postal codes where the product tofu was shipped: USE AdventureWork;
SELECT DISTINCT
 a.PostalCode
FROM
...
-- 32. List of product Names that were shipped to France: USE AdventureWork;
SELECT DISTINCT
 p.Name AS ProductName
FROM
 Sales.SalesOrderDetail sod
...
JOIN
 Production.ProductCategory pc ON psc.ProductCategoryID =
...
-- 35. List of products where units in stock is less than 10 and units on order are
0:
USE AdventureWork;
SELECT
 p.ProductID,
...
-- 37. Number of orders each employee has taken for customers with
CustomerIDs between A and AO:
USE AdventureWork;
SELECT
 e.BusinessEntityID AS EmployeeID,
